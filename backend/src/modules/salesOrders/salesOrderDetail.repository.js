const ORDER_FIELDS = `so.id, so.so_number AS "soNumber", so.order_type AS "orderType", so.customer_id AS "customerId", c.name AS "customerName", so.marketplace, so.tracking_number AS "trackingNumber", so.order_date AS "orderDate", so.deadline, so.priority, so.status, so.created_at AS "createdAt", so.updated_at AS "updatedAt"`;

export async function findDetail(db, salesOrderId) {
