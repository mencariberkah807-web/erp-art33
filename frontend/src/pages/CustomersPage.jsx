...
const EMPTY_FORM = { name: '', company: '', mobile: '', email: '', address: '', customerType: '', status: 'ACTIVE', notes: '' };
const FIELDS = [
...
export default function CustomersPage() {
  ...
  return <section className="page-content">
    ...
    {selected && !modalOpen && <aside className="detail-drawer"><div className="drawer-header"><div><p className="eyebrow">CUSTOMER</p><h2>{selected.name}</h2><span className="subtle">{selected.customerCode}</span></div></div><div className="drawer-body"><div className="detail-status"><span className={`status-pill status-${selected.status.toLowerCase()}`}>{selected.status}</span></div><dl className="detail-list"><dt>Company</dt><dd>{selected.company || '—'}</dd><dt>Mobile</dt><dd>{selected.mobile || '—'}</dd><dt>Email</dt><dd>{selected.email || '—'}</dd><dt>Customer Type</dt><dd>{selected.customerType || '—'}</dd><dt>Address</dt><dd>{selected.address || '—'}</dd><dt>Notes</dt><dd>{selected.notes || '—'}</dd></dl></div><div className="drawer-actions"><button className="secondary-button" type="button" onClick={() => openEdit(selected)}>Edit Customer</button></div></aside>}
    {modalOpen && <EntityFormModal title={editing ? 'Edit Customer' : 'Add Customer'} description={editing ? 'Update the customer master record.' : 'Customer ID is generated automatically when saved.'} fields={FIELDS} values={form} onChange={updateField} onSubmit={submit} onClose={() => !submitting && setModalOpen(false)} submitting={submitting} error={formError} />}
  </section>;
}
