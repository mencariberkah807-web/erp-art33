...
function App() {
  ...
  return <div className="erp-shell">
    ...
    <div className="workspace"><header className="topbar"><div><span className="breadcrumb">ARTKRILIK ERP / {isCustomers || isProducts ? `Master Data / ${activePage}` : isSalesOrders && salesView === 'new' ? 'Sales / New Order' : isProduction ? 'Production / Board' : activePage}</span><h2>{isSalesOrders && salesView === 'new' ? 'New Order' : activePage}</h2></div><div className="topbar-actions"><button className="icon-button" type="button" aria-label="Notifications">●</button><div className="user-chip"><span className="avatar">A</span><span>Admin</span></div></div></header><main className="page-container">
      {isCustomers && <CustomersPage />}{isProducts && <ProductsPage />}{isSalesOrders && salesView === 'list' && <SalesOrdersPage onNewOrder={() => setSalesView('new')} />}{isSalesOrders && salesView === 'new' && <NewOrderPage onSelect={() => {}} onCancel={() => setSalesView('list')} />}{isProduction && <ProductionPage />}{!isCustomers && !isProducts && !isSalesOrders && !isProduction && <PlaceholderPage title={activePage} />}
    </main></div>
  </div>;
}
