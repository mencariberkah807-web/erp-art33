    ...
    <form onSubmit={submit}>
      <div className="form-card"><div className="section-heading"><div><h2>Order Information</h2><p>Transaction identity and delivery deadline.</p></div></div>
        <div className="form-grid">
          <div className="form-field customer-picker-field"><div className="field-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}><span>Customer *</span><button className="secondary-button" type="button" onClick={openCustomerModal}>+ Add Customer</button></div><select value={customerId} onChange={(e) => setCustomerId(e.target.value)} disabled={loadingMaster}><option value="">Select customer...</option>{customers.map((c) => <option key={c.id} value={c.id}>{c.customerCode} — {c.name}</option>)}</select></div>
