...
function App() {
  ...
  useEffect(() => {
    let active = true;
    fetch(`${API_BASE_URL}/health`).then((response) => { if (active) setApiState(response.ok ? 'ok' : 'error'); }).catch(() => { if (active) setApiState('error'); });
    return () => { active = false; };
  }, []);
  ...
  function navigate(page) {
    setActivePage(page);
    if (page === 'Sales Orders') setSalesView('list');
  }

  return (
    <div className="erp-shell">
      <aside className="sidebar">
        <div className="brand"><div className="brand-mark">A</div><div><strong>ARTKRILIK</strong><span>ERP V3</span></div></div>
        <nav className="sidebar-nav" aria-label="Main navigation">
          {['CORE', 'SALES', 'PRODUCTION', 'FULFILLMENT', 'FINANCE', 'MASTER DATA'].map((group) => <div className="nav-group" key={group}><span className="nav-label">{group}</span>{navigation.filter((item) => item.group === group).map((item) => <button className={`nav-item ${activePage === item.label ? 'active' : ''}`} key={item.label} onClick={() => navigate(item.label)} type="button"><span className="nav-icon" />{item.label}</button>)}</div>)}
        </nav>
      ...
      <div className="workspace">
        <header className="topbar"><div><span className="breadcrumb">ARTKRILIK ERP / {isCustomers || isProducts ? `Master Data / ${activePage}` : isSalesOrders && salesView === 'new' ? 'Sales / New Order' : activePage}</span><h2>{isSalesOrders && salesView === 'new' ? 'New Order' : activePage}</h2></div><div className="topbar-actions"><button className="icon-button" type="button" aria-label="Notifications">●</button><div className="user-chip"><span className="avatar">A</span><span>Admin</span></div></div></header>
        <main className="page-container">
          {isCustomers && <CustomersPage />}
          {isProducts && <ProductsPage />}
          {isSalesOrders && salesView === 'list' && <SalesOrdersPage onNewOrder={() => setSalesView('new')} />}
          {isSalesOrders && salesView === 'new' && <NewOrderPage onSelect={() => {}} onCancel={() => setSalesView('list')} />}
          {!isCustomers && !isProducts && !isSalesOrders && <PlaceholderPage title={activePage} />}
        </main>
      </div>
    </div>
  );
