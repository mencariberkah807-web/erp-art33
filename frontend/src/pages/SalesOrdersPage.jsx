...
export default function SalesOrdersPage({ onNewOrder, selectedOrderId, onSelectOrder }) {
  ...
  async function loadOrders() {
    ...
    try {
      ...
      setOrders(body.data || []);
      setMeta(body.meta || { page: 1, pageSize: 20, total: 0 });
    } catch (err) {
  ...
  function selectOrder(id) {
    onSelectOrder(selectedOrderId === id ? null : id);
  }
  ...
  return <section className={`sales-orders-workspace${selectedOrderId ? ' has-detail' : ''}`}>
    ...
    {selectedOrderId && <aside className="sales-order-detail-panel">
      <SalesOrderDetailPage salesOrderId={selectedOrderId} onBack={() => onSelectOrder(null)} embedded />
    </aside>}
  </section>;
}
