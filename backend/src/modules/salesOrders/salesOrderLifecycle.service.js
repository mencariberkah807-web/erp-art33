...
export async function completeSalesOrder(pool, salesOrderId, { isAdmin = true } = {}) {
  ...
  try {
    ...
    if (order.status !== 'HANDOVER') throw error('VALIDATION_ERROR', `Sales order cannot be completed from ${order.status}.`);
    ...
    if (paid < total) throw error('VALIDATION_ERROR', `Sales order payment is not PAID. Balance: ${(total - paid).toFixed(2)}.`);
    ...
    const result = await db.query(`UPDATE sales_orders SET status = 'COMPLETED', updated_at = NOW() WHERE id = $1 AND status = 'HANDOVER' RETURNING id`, [salesOrderId]);
