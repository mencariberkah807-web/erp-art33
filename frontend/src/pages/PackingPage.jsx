...
export default function PackingPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/packing?page=1&pageSize=100`);
  ...
  return <section className="page-section">
    ...
    {loading ? <div className="state-card">Loading packing queue…</div> : rows.length === 0 ? <div className="state-card"><strong>No sales orders ready for packing.</strong><span>Completed production orders will appear here.</span></div> : <div className="table-card"><table><thead><tr><th>SO Number</th><th>Order Date</th><th>Deadline</th><th>Priority</th><th>Status</th><th></th></tr></thead><tbody>{rows.map((row) => <tr key={row.salesOrderId} onClick={() => openDetail(row)}><td><strong>{row.soNumber}</strong></td><td>{row.orderDate || '—'}</td><td>{row.deadline || '—'}</td><td>{row.priority || '—'}</td><td><span className="status-pill status-ready">PACKING</span></td><td><button className="button button-secondary button-small" type="button" onClick={(event) => { event.stopPropagation(); openDetail(row); }}>View</button></td></tr>)}</tbody></table></div>}
    {selected && <div className="drawer-backdrop" onClick={() => setSelected(null)}><aside className="drawer" onClick={(event) => event.stopPropagation()}><div className="drawer-header"><div><p className="eyebrow">PACKING DETAIL</p><h2>{selected.soNumber}</h2></div><button className="icon-button" type="button" onClick={() => setSelected(null)} aria-label="Close">×</button></div>{actionError && <div className="alert alert-error"><span>{actionError}</span></div>}<div className="drawer-body"><div className="detail-grid"><div><span>Order Type</span><strong>{selected.orderType || '—'}</strong></div><div><span>Priority</span><strong>{selected.priority || '—'}</strong></div><div><span>Deadline</span><strong>{selected.deadline || '—'}</strong></div><div><span>Items</span><strong>{selected.items?.length ?? 0}</strong></div></div><div className="detail-section"><h3>Items</h3>{(selected.items || []).map((item) => <div className="detail-row" key={item.id}><div><strong>{item.productName || 'Product'}</strong><span>Qty {item.quantity}</span></div><span>{item.status}</span></div>)}</div><div className="detail-section"><h3>Work Orders</h3>{(selected.workOrders || []).map((wo) => <div className="detail-row" key={wo.id}><div><strong>{wo.woNumber}</strong><span>{wo.status}</span></div></div>)}</div></div><div className="drawer-footer"><button className="button button-secondary" type="button" onClick={() => setSelected(null)}>Close</button><button className="button button-primary" type="button" onClick={packOrder} disabled={packing}>{packing ? 'Packing…' : 'Pack'}</button></div></aside></div>}
  </section>;
}
